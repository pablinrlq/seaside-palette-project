...
export function CatalogProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (ready) localStorage.setItem(STORAGE_KEY, JSON.stringify({ products: catalogProducts, categories }));
  }, [catalogProducts, categories, ready]);

  const value = useMemo<CatalogContextValue>(() => ({
    products: catalogProducts,
    categories,
    saveProduct: product => setCatalogProducts(old => old.some(item => item.id === product.id) ? old.map(item => item.id === product.id ? product : item) : [product, ...old]),
    deleteProduct: id => setCatalogProducts(old => old.filter(item => item.id !== id)),
    duplicateProduct: id => setCatalogProducts(old => {
      const source = old.find(item => item.id === id);
      if (!source) return old;
      const stamp = Date.now().toString();
      return [{ ...source, id: stamp, slug: `${source.slug}-copia-${stamp.slice(-4)}`, name: `${source.name} — cópia`, featured: false }, ...old];
    }),
    addCategory: name => {
      const clean = name.trim();
      if (!clean || categories.some(category => category.toLowerCase() === clean.toLowerCase())) return false;
      setCategories(old => [...old, clean]);
      return true;
    },
    renameCategory: (oldName, newName) => {
      const clean = newName.trim();
      if (!clean || categories.some(category => category !== oldName && category.toLowerCase() === clean.toLowerCase())) return false;
      setCategories(old => old.map(category => category === oldName ? clean : category));
      setCatalogProducts(old => old.map(product => product.category === oldName ? { ...product, category: clean } : product));
      return true;
    },
    deleteCategory: name => {
    ...
    resetCatalog: () => { setCatalogProducts(initialProducts); setCategories(defaultCategories); },
    importCatalog: data => {
      ...
      const valid = parsed.products.every(product => product.id && product.name && product.slug && product.category && typeof product.price === "number");
